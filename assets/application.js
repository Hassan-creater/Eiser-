...
function showProduct(data) {
  ...
  container.innerHTML = `
    ...
    <div class="w-full px-[5em] mx-auto my-8">
      <div class="grid grid-cols-1 md:grid-cols-2 gap-[2em]">
        ...
        <div class="product--information sticky top-5 py-2 pl-[3em]">
          <form method="post" action="/cart/add.js" class="add-to-cart-form" data-product-id="${data.id}" novalidate>
            <input type="hidden" name="id" value="${data.id}">
            ...
            <div class="my-3" id="price-section">
              ${data.compare_at_price ? `<span class="text-base text-gray-500 line-through">$${data.compare_at_price}</span>` : ''}
              <span class="text-[24px] text-green-500">$${data.price}</span>
            </div>

            <div class="px-2 py-[2em] flex flex-col gap-3 border-b-[1px] border-black">
              <div class="flex gap-9"><p>Category</p><p class="text-green-400">: ${data.category}</p></div>
              <div class="flex gap-9"><p>Availability</p><p>: ${data.available ? 'In Stock' : 'Out of Stock'}</p></div>
            </div>
            ...
            <div class="my-[3em]"><p class="text-gray-700">${data.description}</p></div>

            <div class="my-3 flex gap-[2em] items-center">
              
              <form class="add-to-cart-form" data-product-id="${data.id}" >
            <input type="hidden" name="id" value="${data.id}">
           <button type="submit" class="add-to-cart-button w-[10em] px-6 py-2 bg-lime-500 border-[1px] border-lime-500 text-white hover:bg-transparent duration-300 hover:text-lime-500 rounded-sm my-2" ${!data.available ? 'disabled' : ''}>
                ${data.available ? 'Add to cart' : 'Sold out'}
              </button>
        </form>
  ...
  // Re-bind newly injected forms and buttons
  initCartForms();
  initWishlistButtons();
}
